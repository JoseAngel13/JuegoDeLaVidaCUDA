...
double GOLdouble(int tamanio, int iteraciones)
{
	 ...
	 *  
 	*/
	leeMdouble(malla, aux, tamanio);
	//imprimeMdouble(malla, N);
	//AQUI INICIAMOS A CONTAR EL TIEMPO
	inicio=clock();
	for (int i = 0; i < iteraciones; ++i)
	{
		actualizadouble(malla, aux, N);
		//printf("Iteracion %d\n", i + 1);
		//imprimeMdouble(malla, N);
	}
	final = clock();
...
void leeMdouble(double *malla, double *aux, int tamanio)
{
 ...
 */
void imprimeMdouble(double *m, int N)
{
	for (int i = 0; i < N; ++i)
	{
		for (int j = 0; j < N; ++j)
 ...
 */
void actualizadouble(double *malla, double *aux, int N)
{
	int contador = 0;
	int celActual;
	for (int i = 0; i < N; ++i)
