...
double GOLlong(int tamanio, int iteraciones)
{
	 ...
	 *  
 	*/
 	leeMlong(malla,aux,tamanio);
	//imprimeMlong(malla, N);
	//AQUI INICIAMOS A CONTAR EL TIEMPO
	inicio=clock();
	for (int i = 0; i < iteraciones; ++i)
	{
		actualizalong(malla, aux, N);
		//printf("Iteracion %d\n", i + 1);
		//imprimeMlong(malla, N);
	}
	final = clock();
...
void leeMlong(long *malla, long *aux,int tamanio)
{
 ...
 */
void imprimeMlong(long *m,int N)
{
	for (int i = 0; i < N; ++i)
	{
		for (int j = 0; j < N; ++j)
 ...
 */
void actualizalong(long *malla, long *aux, int N)
{
	int contador = 0;
	int celActual;
	for (int i = 0; i < N; ++i)
