#include "funcionesParalelo_long.h"
#include <math.h>
double GOL_Plong(int N, int iteraciones, int MAX_threads){
    long *tablero, *tablero_aux;
	long *d_tablero, *d_tablero_aux;
	clock_t  inicio, final;
    size_t size = N*N*sizeof(long);

    //Asignacion de memoria del lado del host
    tablero = (long*)malloc(size);
    tablero_aux = (long*)malloc(size);
	...
	//imprimeM_P_long<<<1,1>>>(d_tablero);
	cudaThreadSynchronize();
	for (int i = 0; i < iteraciones; ++i)
	{
	 	//printf("Iteracion %d\n",i+1);
		actualiza_Plong<<<dimGrid,dimBlock>>>(d_tablero, d_tablero_aux, N, dim_MAX);
		cudaThreadSynchronize();//espera a que todos los hilos terminen su ejecución
		copiaMatriz_Plong<<<dimGrid,dimBlock>>>(d_tablero_aux, d_tablero, N, dim_MAX);
		cudaThreadSynchronize();
		//imprimeM_Plong<<<1,1>>>(d_tablero,N);
		//cudaThreadSynchronize();
	}
	final = clock();
