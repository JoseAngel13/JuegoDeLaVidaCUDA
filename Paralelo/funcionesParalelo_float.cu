#include "funcionesParalelo_float.h"
#include <math.h>
//#define trheads 768*6 // cuda cores y SM 

double GOL_Pfloat(int N, int iteraciones, int MAX_threads){
    float *tablero, *tablero_aux;
	float *d_tablero, *d_tablero_aux;
	clock_t  inicio, final;
    size_t size = N*N*sizeof(float);

    //Asignacion de memoria del lado del host
    tablero = (float*)malloc(size);
    tablero_aux = (float*)malloc(size);
    ...
    */	
	...
	//imprimeM_Pfloat<<<1,1>>>(d_tablero,N);
	//cudaThreadSynchronize();
	for (int i = 0; i < iteraciones; ++i)
	{
	 	//printf("Iteracion %d\n",i+1);
		actualiza_Pfloat<<<dimGrid,dimBlock>>>(d_tablero,d_tablero_aux,N, dim_MAX);
		cudaThreadSynchronize();//espera a que todos los hilos terminen su ejecución
		copiaMatriz_Pfloat<<<dimGrid,dimBlock>>>(d_tablero_aux,d_tablero,N,dim_MAX);
		cudaThreadSynchronize();
		//imprimeM_Pfloat<<<1,1>>>(d_tablero,N);
		//cudaThreadSynchronize();
	}
