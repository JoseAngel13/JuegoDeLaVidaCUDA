#include "funcionesParalelo_int.h"
#include <math.h>
double GOL_Pint(int N, int iteraciones, int MAX_threads){
    ...
    */	
	...
	//imprimeM_Pint<<<1,1>>>(d_tablero,N);
	//cudaThreadSynchronize();
	for (int i = 0; i < iteraciones; ++i)
	{
	 	//printf("Iteracion %d\n",i+1);
		actualiza_Pint<<<dimGrid,dimBlock>>>(d_tablero,d_tablero_aux,N,dim_MAX);
		cudaThreadSynchronize();//espera a que todos los hilos terminen su ejecución
		copiaMatriz_Pint<<<dimGrid,dimBlock>>>(d_tablero_aux,d_tablero,N,dim_MAX);
		cudaThreadSynchronize();
		//imprimeM_Pint<<<1,1>>>(d_tablero,N);
		//cudaThreadSynchronize();
	}
